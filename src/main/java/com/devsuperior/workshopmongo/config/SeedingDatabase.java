...
public class SeedingDatabase implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		Mono<Void> deleteUser = userRepository.deleteAll();
		...
		Flux<User> isertUser = userRepository.saveAll(Arrays.asList(maria, alex, bob));
		isertUser.subscribe();
		...
		Flux<Post> isertPosts = postRepository.saveAll(Arrays.asList(post1, post2));
		isertPosts.subscribe();
